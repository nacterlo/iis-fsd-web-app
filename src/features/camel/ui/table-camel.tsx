...
import { useGetCamelListQuery } from "../api/camel-api"
...
export const TableCamel = ({ onOpenModalUpdate }: TableProps) => {
    const [page, _setPage] = useState(1)
    const limit = 10

    const { data: camelsList, isLoading: loadingCamels, isError } = useGetCamelListQuery({ limit, page })
    ...
    return (
        ...
        >
            ...
            <Table
                ...
                stickyHeader
                hoverRow={camelsList && !loadingCamels|| !isError || !!camelsList?.data}
                sx={{
            ...
            >
                ...
                {camelsList && camelsList.data ? (
                    <tbody>
                        {camelsList.data.length === 0 ? (
                            <TableNoData />
                        ) : null}
                        {[...camelsList.data].map((animal, index) => (
