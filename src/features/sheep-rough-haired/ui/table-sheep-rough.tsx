...
import { useGetSheepRoughListQuery } from "../api/sheep-rough-api"
interface TableProps {
    onOpenModalUpdate: (id: number) => void
}

export const TableSheepRough = ({ onOpenModalUpdate }: TableProps) => {
    const [page, _setPage] = useState(1)
    const limit = 100

    const { data: sheepList, isLoading: loadingSheepList, isError } = useGetSheepRoughListQuery({ limit, page })
    ...
    return (
        ...
        >
            ...
            <Table
                ...
                stickyHeader
                hoverRow={sheepList && !loadingSheepList || !isError || !!sheepList?.data}
                sx={{
            ...
            >
                ...
                {sheepList && sheepList.data ? (
                    <tbody>
                        {sheepList.data.length === 0 ? (
                            <TableNoData />
                        ) : null}
                        {[...sheepList.data].map((animal, index) => (
