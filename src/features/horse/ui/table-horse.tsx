...
import { useGetHorseListQuery } from "../api/horse-api"
...
export const TableHorse = ({ onOpenModalUpdate }: TableProps) => {

    const [page, _setPage] = useState(1)
    const limit = 10

    const { data: horseList, isLoading: loadingHorse, isError } = useGetHorseListQuery({ limit, page })
    ...
    return (
        ...
        >
            ...
            <Table
                ...
                stickyHeader
                hoverRow={horseList && !loadingHorse || !isError || !!horseList?.data}
                sx={{
            ...
            >
                ...
                {horseList && horseList.data ? (
                    <tbody>
                        {horseList.data.length === 0 ? (
                            <TableNoData />
                        ) : null}
                        {[...horseList.data].map((animal, index) => (
