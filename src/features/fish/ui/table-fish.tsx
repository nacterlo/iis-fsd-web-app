...
import { useGetFishListQuery } from "../api/fish-api"
    ...
    return (
        ...
        >
            ...
            <Table
                ...
                stickyHeader
                hoverRow={furryList && !loadingFurry || !isError || !!furryList?.data}
                sx={{
            ...
            >
                ...
                {furryList && furryList.data ? (
                    <tbody>
                        {furryList.data.length === 0 ? (
                            <TableNoData />
                        ) : null}
                        {[...furryList.data].map((animal, index) => (
                            <tr key={index} style={{ cursor: 'pointer' }} onClick={() => onOpenModalUpdate(animal.id)}>
                                <td>
                                    <Typography level="title-sm">{animal.name}</Typography>
                                </td>
                                <td>
