...
import { useGetDeerListQuery } from "../api/deer-api"
...
export const TableDeer = ({ onOpenModalUpdate }: TableProps) => {

    const [page, _setPage] = useState(1)
    const limit = 10

    const { data: deersList, isLoading: loadingDeers, isError } = useGetDeerListQuery({ limit, page })
    ...
    return (
        ...
        >
            ...
            <Table
                ...
                stickyHeader
                hoverRow={deersList && !loadingDeers || !isError || !!deersList?.data}
                sx={{
            ...
            >
                ...
                {deersList && deersList.data ? (
                    <tbody>
                        {deersList.data.length === 0 ? (
                            <TableNoData />
                        ) : null}
                        {[...deersList.data].map((animal, index) => (
