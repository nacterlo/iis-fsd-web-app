...
import { useGetPigListQuery } from "../api/pig-api"
...
interface TablePigProps {
    onOpenModalUpdate: (id: number) => void
}

export const TablePig = ({ onOpenModalUpdate }: TablePigProps) => {

    const [page, _setPage] = useState(1)
    const limit = 10

    const { data: pigsList, isLoading: loadingPigs, isError } = useGetPigListQuery({ limit, page })
    ...
    return (
        ...
        >
            ...
            <Table
                ...
                stickyHeader
                hoverRow={pigsList && !loadingPigs || !isError || !!pigsList?.data}
                sx={{
            ...
            >
                ...
                {pigsList && pigsList.data ? (
                    <tbody>
                        {pigsList.data.length === 0 ? (
                            <TableNoData />
                        ) : null}
                        {[...pigsList.data].map((animal, index) => (
