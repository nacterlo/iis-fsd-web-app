...
import { SheepSmushkovy, UpdateSheepSmushkovy } from "../../model/types";

interface UpdateFormProps {
    initialData: SheepSmushkovy
    onSubmitUpdate: (data: UpdateSheepSmushkovy) => void
}

export const FormUpdateSheepSmushkovy = ({ initialData, onSubmitUpdate }: UpdateFormProps) => {
    const {
        control,
        handleSubmit,
        reset,
    ...
    const onSubmit: SubmitHandler<UpdateSheepSmushkovy> = (data) => {
        onSubmitUpdate(data)
    }
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} id='updateSheepSmushkovy'>
            <Stack direction='column' spacing={1.5}>
                ...
                <FormControl sx={{ width: '100%' }}>
                    <FormLabel required>Селекционируемые признаки</FormLabel>
                    <Controller
                        name="selectedCharacteristics"
                        control={control}
                        rules={{ required: 'Обязательное поле' }}
                        render={({ field }) => (
                            <Textarea size="sm" minRows={2} placeholder='Селекционируемые признаки' {...field} />
                        )}
                    />
                </FormControl>
                <FormControl sx={{ width: '100%' }}>
