...
import { useGetEggListQuery } from "../api/egg-api"
    ...
    return (
        ...
        >
            ...
            <Table
                ...
                stickyHeader
                hoverRow={eggList && !loadingEggList || !isError || !!eggList?.data}
                sx={{
            ...
            >
                <thead>
                    <tr>
                        <th style={{ width: '20%', padding: '12px 6px', color: '#2e93fe' }}>Кросс</th>
                        <th style={{ width: '20%', padding: '12px 6px' }}>Линия</th>
                        <th style={{ width: '15%', padding: '12px 6px' }}>Дата создания</th>
                        <th style={{ width: '35%', padding: '12px 6px', textAlign: 'center' }}>Статус</th>
                    </tr>
                </thead>
                {eggList && eggList.data ? (
                    <tbody>
                        {eggList.data.length === 0 ? (
                            <TableNoData />
                        ) : null}
                        {[...eggList.data].map((animal, index) => (
                            ...
                            <tr key={index} style={{ cursor: 'pointer' }} onClick={() => onOpenModalUpdate(animal.id)}>
                                <td>
                                    <Typography level="title-sm">{animal.cross}</Typography>
                                </td>
                                <td>
                                    <Typography level="body-xs">{animal.line}</Typography>
                                </td>
                                <td>
